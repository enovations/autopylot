...
class rumbaLED:
	...
	def Set(self, LEDindex, r, g, b):
		self.LEDdata[LEDindex] = (r/5) + ((g/5) << 8) + ((b/5) << 16)

	def SetAll(self, r, g, b):
		for i in range(50):
			self.LEDdata[i] = (r/5) + ((g/5) << 8) + ((b/5) << 16)
...
#		while True:
#			for i in range(W):
#				LED.Set(i, 100*(i%2), 100*(i%2), 100*(i%2))
#			LED.Update()
#
#			time.sleep(0.1)
#			for i in range(W):
#				LED.Set(i, 100*((i+1)%2), 100*((i+1)%2), 100*((i+1)%2))
#			LED.Update()
#
#			time.sleep(0.1)
